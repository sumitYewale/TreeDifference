...
class TreeDifference{    
    ...
    public static function get_diff($structured_arr1 , $structured_arr2 , $status){
        try{
					...
					if(is_array($val) && is_array($old[$key])){
						if(count($val) == count($old[$key])){
							if((count($val) != count($val , COUNT_RECURSIVE))){
								self::edit_values_in_array($val , $old[$key]);
							}		
							else{
								$differ = [];
								...
								if(!empty($differ) && $val != [] && !empty($val) && $val != null){
									$arrDiff[$key] = (object)[
										'oldvalue' => (object)$old[$key],
										'newvalue' => (object)$val,
									];

                                    if($status) $arrDiff[$key]->difference = $differ;
								}
							}
						}
						else{
